...
// Carta pública por QR (sin login)
...
Route::middleware(['auth', 'tenant'])->group(function () {
    ...
    Route::middleware('suscrito')->group(function () {
        ...
        // Facturación electrónica (multi-país: SUNAT / DIAN / SII / ARCA / SAT)
        Route::get('/facturacion', [\App\Http\Controllers\Modules\FacturacionController::class, 'index'])->name('facturacion.index');
        Route::post('/facturacion/pedido/{pedido}', [\App\Http\Controllers\Modules\FacturacionController::class, 'emitir'])->name('facturacion.emitir');
        Route::post('/facturacion/{comprobante}/anular', [\App\Http\Controllers\Modules\FacturacionController::class, 'anular'])->name('facturacion.anular');
        Route::post('/facturacion/resumen', [\App\Http\Controllers\Modules\FacturacionController::class, 'resumen'])->name('facturacion.resumen');
        Route::get('/facturacion/{comprobante}/imprimir', [\App\Http\Controllers\Modules\FacturacionController::class, 'imprimir'])->name('facturacion.imprimir');
        ...
        Route::middleware('role:admin')->group(function () {
            ...
            Route::get('/facturacion/configuracion', [\App\Http\Controllers\Modules\FacturacionConfigController::class, 'edit'])->name('facturacion.config.edit');
            Route::put('/facturacion/configuracion', [\App\Http\Controllers\Modules\FacturacionConfigController::class, 'update'])->name('facturacion.config.update');
        });
    });
});
...
/*
|--------------------------------------------------------------------------
| Solo entorno local: regenerar datos de demostración desde el navegador.
| Visita /dev/demo-data (logueado) y vuelve al dashboard con datos de HOY.
|--------------------------------------------------------------------------
*/
if (app()->environment('local')) {
    Route::get('/dev/demo-data', function () {
        \Illuminate\Support\Facades\Artisan::call('db:seed', ['--class' => 'DemoDataSeeder', '--force' => true]);
        return redirect()->route('dashboard')->with('status', 'Datos demo regenerados con fecha de hoy.');
    })->middleware('auth')->name('dev.demo');
}
